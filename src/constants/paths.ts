// Relative path to token endpoint in authenticatino server.
export const AUTH_TOKEN_ENDPOINT = "/connect/token";

// Relative paths of various API endpoints.
export const NCC_PATHS = {
    REALTIME_API: "/realtime",
    NODE_DOMAIN: "/realtime/domain"
};

//export const DEFAULT_API_WS_ORIGIN = "wss://api.noccela.com";
//export const DEFAULT_API_HTTP_ORIGIN = "https://api.noccela.com";
//export const DEFAULT_AUTH_ORIGIN = "https://auth.noccela.com";

export const DEFAULT_API_WS_ORIGIN = "ws://api.jp.noccela.xyz";
export const DEFAULT_API_HTTP_ORIGIN = "http://api.jp.noccela.xyz";
export const DEFAULT_AUTH_ORIGIN = "http://auth.jp.noccela.xyz";
